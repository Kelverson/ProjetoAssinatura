...
def Criar_Assinatura():
    ...
    if(repost != True):
        ...
        for account in accounts:
            ...
            if(account.DisplayName.split('@')[1] == 'newline.ind.br' or account.DisplayName.split('@')[1] == 'newstandard.ind.br'):
                ...
                response = requests.get(
                    url=f"https://graph.microsoft.com/beta/users('{account.DisplayName}')/profile",
                ...
                if(response.status_code == 200):
                    ...
                    try:
                        ...
                        #inline_shape.PictureFormat.ApplyPictureStyle(4)################################################# TESTAR
                        ...
                        # Adicionar um parágrafo abaixo do título
                        range_ = doc.Range(doc.Content.End - 1, doc.Content.End)
                        if(filtered_data["officeLocation"] != None):
                        ...
                        # Adicionar um parágrafo abaixo do título
                        range_ = doc.Range(doc.Content.End - 1, doc.Content.End)
                        if(filtered_data["state"] != None):
                            range_.Text = filtered_data["city"]+"-"+filtered_data["state"]+" | "+filtered_data["postalCode"]+" | Brasil" #Removido pois a API GRAPH tirou o conteudo do campo "state" 
                        else:
                            range_.Text = filtered_data["city"]+" - SP | "+filtered_data["postalCode"]+" | Brasil"
                        #range_.Font.family = 'verdana'
                        range_.Font.Size = 9.5
                        range_.Font.Bold = False
                        range_.InsertParagraphAfter()
                        ...
                        caminho = os.path.join(diretorio, f'Assinatura ({account.DisplayName}).htm')
...
if(outlook_installed == True and word_installed == True):
    Criar_Assinatura()